...
const DATA_DIR = process.env.WEB_TERMINAL_DATA_DIR
  ? path.resolve(process.env.WEB_TERMINAL_DATA_DIR)
  : path.join(__dirname, "..", ".web-terminal");
...
function touchRecentSession(dirPath, meta = {}) {
  if (!dirPath) return;
  upsertRecentDirEntry(dirPath, (entry, now) => {
    if (entry.lastLaunch && meta.sessionName) {
      entry.lastLaunch.sessionName = meta.sessionName;
    }
    entry.historyMeta = {
      ...(entry.historyMeta || {}),
      lastActiveAt: now,
...
function removeRecentDirEntry(dirPath) {
  if (!dirPath) return false;
  const data = getRecentDirs();
  const before = data.entries.length;
  data.entries = data.entries.filter((entry) => entry.path !== dirPath);
